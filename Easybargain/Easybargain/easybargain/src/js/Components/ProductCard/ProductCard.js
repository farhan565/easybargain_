import React, { Component } from 'react';
import "./ProductCard.css";
import BidTimer from '../BitTimer/BidTimer';
import BidHistory from '../BidHistory/BidHistory';
import axios from "axios";
import Web3 from 'web3';


export default class ProductCard extends React.Component {

    constructor(props) {
        super(props)
        this.state = {
            pricesubmitted: false,
            price: "",
            text: "",
            timeEnded: false,
            Bids: [],
        }
        this.handleDataChange = this.handleDataChange.bind(this);
        this.bidProduct = this.bidProduct.bind(this);

            const wa = window;
            this.web3 = null;
            this.contract = null;
            this.account = null;
        
            if (window.hasOwnProperty('web3')) {
              let ethereum = wa.ethereum;
              ethereum.enable();
              this.web3 = new Web3();
              this.web3.setProvider(wa.web3.currentProvider);
        
              var address = "0x4Adb4797BE62E4443a3cabb9AF2C1ED47B4Ee472";
              this.contract = new this.web3.eth.Contract([
                {
                  "constant": false,
                  "inputs": [
                    {
                      "name": "_id",
                      "type": "string"
                    },
                    {
                      "name": "name",
                      "type": "string"
                    },
                    {
                      "name": "productName",
                      "type": "string"
                    },
                    {
                      "name": "email",
                      "type": "string"
                    },
                    {
                      "name": "price",
                      "type": "string"
                    }
                  ],
                  "name": "AddOrder",
                  "outputs": [],
                  "payable": false,
                  "stateMutability": "nonpayable",
                  "type": "function"
                },
                {
                  "constant": true,
                  "inputs": [
                    {
                      "name": "",
                      "type": "uint256"
                    }
                  ],
                  "name": "bid_winners_list",
                  "outputs": [
                    {
                      "name": "id",
                      "type": "uint256"
                    },
                    {
                      "name": "_id",
                      "type": "string"
                    },
                    {
                      "name": "name",
                      "type": "string"
                    },
                    {
                      "name": "productName",
                      "type": "string"
                    },
                    {
                      "name": "email",
                      "type": "string"
                    },
                    {
                      "name": "price",
                      "type": "string"
                    }
                  ],
                  "payable": false,
                  "stateMutability": "view",
                  "type": "function"
                }
              ], address);
              this.web3.eth.getAccounts().then((accounts) => {
                this.account = accounts[0];
                // console.log(accounts);
              });
              console.log(this.contract);
        
              // console.log(contract)
            } else {
              alert('No Metamask Extension installed. You need to install metamask in order to secure your Biding...');
            }
          
        
        

    }

    componentDidMount() {

    }



    render() {


        if(this.state.timeEnded){
          
          axios.get("http://localhost:3003/product/view/"+this.props.title).then((res)=>{

                let bidHistory =res.data.bidhistory;
                console.log(bidHistory);
                if(bidHistory){
                    if(bidHistory.length>0){
                    
                        let winner = bidHistory[0];
                      
                        
                        for(let i=1; i<bidHistory.length;i++){
                            let maxPrice = parseInt(bidHistory[0].price);
                      
                            if(bidHistory[i].price > maxPrice){
                                 maxPrice = bidHistory[i].price;
                                 winner = bidHistory[i]
                            }

                        }
                        

                        console.log(winner.username)
                        if(winner.username  == localStorage.userName){
                            alert("You are the winner")

                            let Winner = {
                                name :winner.username,
                                prodName: this.props.title
                            }

                            axios.get("http://localhost:3003/bidder/authenticateuser/"+winner.username).then((resp)=>{
                                Winner = {
                                    name :winner.username,
                                    prodName: this.props.title,
                                    email:resp.data.email,
                                    price:winner.price

                                }

                                

                            axios.post("http://localhost:3003/product/add/winner", Winner).then((res)=>{
                                this.contract.methods.AddOrder(res.data["winner"]._id, res.data["winner"].name, res.data["winner"].prodName, res.data["winner"].email, res.data["winner"].price).send({ from: this.account }).then(function (result) {
                                    console.log(result);
                                    console.log("DONE");
                                 });
                                console.log(res.data["winner"]._id)
                                console.log(this.props.title)
                                this.props.navigationProps.push('/order',{productName:this.props.title})
                            })
                            })


                        }

                }
                else{

                }
                }
                

          })  


        }


        // console.log(this.state.timeEnded)

        return (<div className="ProductCard">

            <img src="data:image/jpeg;base64," className="ProductCard__img" />
            <p className="ProductCard__title">{this.props.title}</p>
            <p className="ProductCard__price">BasePrice:{"   " + this.props.price}</p>
            <p className="ProductCard__description">{this.props.description}</p>
            <BidTimer timeFromServer={30} sendData={this.handleDataChange} />
            {!this.state.pricesubmitted && !this.state.timeEnded &&
                <div>
                    <input className="bidenterfield" type="number" placeholder="Enter Bid here" onChange={(e) => { this.setState({ price: e.target.value }) }} />
                    <button className="ProductCard__bidBtn" onClick={() => {
                        this.bidProduct()
                    }}>Bid</button>
                </div>
            }
            {this.state.pricesubmitted &&
                <div>
                    <button className="ProductCard__bidBtn" onClick={() => { this.setState({ pricesubmitted: false }) }}>Re Bid</button>
                    <BidHistory title={this.props.title} />
                </div>
            }

        </div>)
    }


    handleDataChange(data) {
        this.setState({ timeEnded: data })
    }


    async  bidProduct() {

        let tempArray = [];
        let HighestBid = 0;

        await axios.get("http://localhost:3003/product/view/" + this.props.title).then((data) => {
            this.setState({ Bids: data.data });
        })

        if (!(this.state.price == "") && !(this.props.username == "")) {
  
            tempArray = this.state.Bids.bidhistory;
            
     

            if(tempArray.length > 0){
                HighestBid = parseFloat(tempArray[0].price)
                for (let i = 0; i < tempArray.length; i++) {
                    if (parseFloat(tempArray[i].price) > HighestBid) {
                        HighestBid = parseFloat(tempArray[i].price)
                    }

                }
            }
            if (tempArray.length <= 0 || this.state.price > HighestBid
                ) {

                tempArray.push({ username: this.props.username, price: this.state.price })
                // console.log(tempArray)
                await axios.post("http://localhost:3003/product/updateproduct", { prodName: this.props.title, bidhistory: tempArray }).then((data) => {
                    // console.log(this.state.Bids)
                    this.setState({ pricesubmitted: true })
                })
            }
            console.log("hig: ", HighestBid)
            
        }

        else {
            this.setState({ pricesubmitted: false })
        }

        
    }



}